...
from scene.colmap_loader import (
    ...
    write_cameras_binary_slam,
    write_cameras_text_slam, 
    write_images_binary_slam,
    write_images_text_slam, 
)
...
def save_extrinsic_slam(sparse_path, img_idx, extrinsics_w2c, img_files, image_suffix):
    images_bin_file = sparse_path / 'images.bin'
    images_txt_file = sparse_path / 'images.txt'
    
    if images_txt_file.exists():
        with images_txt_file.open('r') as f:
            lines = f.readlines()
        # Assume the first 4 lines are header, and each camera record occupies two lines.
        if len(lines) > 4:
            data_lines = lines[4:]
            # Get the first line of the last camera record (each record is 2 lines)
            last_camera_line = data_lines[-2].strip()  # first line of the last record
            parts = last_camera_line.split()
            if len(parts) >= 8:
                # Expected format:
                # IMAGE_ID, QW, QX, QY, QZ, TX, TY, TZ, CAMERA_ID, NAME
                # Extract quaternion (indices 1 to 4) and translation (indices 5 to 7)
                prev_qvec = [float(x) for x in parts[1:5]]
                prev_tvec = [float(x) for x in parts[5:8]]
                print("Latest camera pose from images.txt:")
                print("Quaternion:", prev_qvec)
                print("Translation:", prev_tvec)
                # Convert the stored quaternion into a rotation matrix
                prev_abs_R = qvec2rotmat(prev_qvec)
                prev_abs_t = np.array(prev_tvec)
            else:
                print("Last camera record in images.txt is not in the expected format.")
        else:
            print("images.txt exists but does not contain any camera records yet.")
    else:
        print("images.txt does not exist; this will be the first camera record.")
    
    
    images = {}
    
    w2c = extrinsics_w2c[-1]
    ...
    R_abs = np.dot(prev_abs_R, rotation_matrix)
    tvec = prev_abs_t + np.dot(prev_abs_R, tvec)
    
    # Convert the absolute rotation matrix into a quaternion.
    qvec = rotmat2qvec(R_abs)
        
    images[img_idx] = BaseImage(
        id=img_idx,
        qvec=qvec,
        tvec=tvec,
        camera_id=img_idx,
    ...
    write_images_binary_slam(images, images_bin_file, img_idx)
    write_images_text_slam(images, images_txt_file, img_idx)
...
def save_intrinsics_slam(sparse_path, img_idx, focals, org_imgs_shape, imgs_shape, save_focals=False):
    cameras_bin_file = sparse_path / 'cameras.bin'
    cameras_txt_file = sparse_path / 'cameras.txt'
    org_width, org_height = org_imgs_shape
    scale_factor_x = org_width / imgs_shape[2]
    ...
    # If cameras.txt exists, read the last focal value (scaled) and recover the original focal.
    if cameras_txt_file.exists():
        with cameras_txt_file.open('r') as f:
            lines = [line.strip() for line in f if line.strip()]
        if lines:
            last_line = lines[-1]
            # Line is whitespace-separated and has the following fields:
            # id, model, width, height, focal_x, focal_y, cx, cy
            parts = last_line.split()
            # Extract the first focal value that was scaled (i.e. focal_x).
            focal_scaled = float(parts[4])
            # Recover original focal by dividing by scale_factor_x.
            focals = focal_scaled / scale_factor_x
        else:
            focals = focals[0]
    else:
        focals = focals[0]
    ...
    cameras = {}
    cameras[img_idx] = Camera(
        id=img_idx,
        model="PINHOLE",
        width=org_width,
        height=org_height,
        params=[focals*scale_factor_x, focals*scale_factor_y, org_width/2, org_height/2]
        )    
    print(f' - scaling focal: ({focals}, {focals}) --> ({focals*scale_factor_x}, {focals*scale_factor_y})' )
    write_cameras_binary_slam(cameras, cameras_bin_file, img_idx)
    write_cameras_text_slam(cameras, cameras_txt_file, img_idx)
    if save_focals:
        np.save(sparse_path / 'non_scaled_focals.npy', focals)
...
def save_points3D_slam(sparse_path, img_idx, imgs, pts3d, confs, masks=None, use_masks=True, 
                         save_all_pts=False, save_txt_path=None, depth_threshold=0.1, 
                         max_pts_num=150 * 10**10):
    """
    Process 3D points, colors, and confidences and save them into multiple files.
    For each output file, if it already exists the new data is appended (or merged) with the existing data,
    while preserving the file structures.
    
    Assumes that:
      - to_numpy(x) converts input x to a NumPy array.
      - storePly(path, xyz, rgb) writes a PLY file with vertices (xyz) and colors (rgb).
    """

    # Define output file paths.
    points3D_ply_file = sparse_path / 'points3D.ply'
    conf_file = sparse_path / 'confidence.npy'
    conf_dsp_file = sparse_path / 'confidence_dsp.npy'
    ...
    # Save raw confidence data (append if file exists).
    if confs is not None:
        if conf_file.exists():
            existing_confs = np.load(conf_file)
            combined_confs = np.concatenate([existing_confs, confs], axis=0)
            np.save(conf_file, combined_confs)
        else:
    ...
    if use_masks:
        ...
        pts = np.concatenate([p[m] for p, m in zip(pts3d, masks)])
        col = np.concatenate([p[m] for p, m in zip(imgs, masks)])
        # For confidences, reshape the mask if needed.
        confs = np.concatenate([p[m] for p, m in zip(confs, masks.reshape(masks.shape[0], -1))])
    else:
        pts = np.array(pts3d)
        col = np.array(imgs)
    ...
    pts = pts.reshape(-1, 3)
    col = col.reshape(-1, 3) * 255.  # Scale colors to 0-255.
    confs = confs.reshape(-1, 1)

    co_mask_dsp_pts_num = pts.shape[0]
    
    # Downsample new batch if exceeding max_pts_num.
    if pts.shape[0] > max_pts_num:
        print(f"Downsampling new points from {pts.shape[0]} to {max_pts_num}")
        confs_min = np.min(confs)
        confs_max = np.max(confs)
        confs = (confs - confs_min) / (confs_max - confs_min) + 1
        weights = confs.reshape(-1) / np.sum(confs)
        indices = np.random.choice(pts.shape[0], max_pts_num, replace=False, p=weights)
        pts = pts[indices]
        col = col[indices]
        ...
        conf_dsp_pts_num = co_mask_dsp_pts_num

    # Save downsampled confidences (append if file exists).
    if confs is not None:
        if conf_dsp_file.exists():
            existing_conf_dsp = np.load(conf_dsp_file)
            combined_conf_dsp = np.concatenate([existing_conf_dsp, confs], axis=0)
            np.save(conf_dsp_file, combined_conf_dsp)
        else:
            np.save(conf_dsp_file, confs)

    # Update or create the points3D.ply file along with its associated downsampled confidences.
    if points3D_ply_file.exists():
        # Read existing PLY file.
        existing_ply = PlyData.read(str(points3D_ply_file))
        existing_vertices = existing_ply['vertex'].data
        existing_xyz = np.vstack((existing_vertices['x'], 
                                  existing_vertices['y'], 
                                  existing_vertices['z'])).T
        existing_rgb = np.vstack((existing_vertices['red'], 
                                  existing_vertices['green'], 
                                  existing_vertices['blue'])).T
        # Load existing downsampled confidences.
        if conf_dsp_file.exists():
            existing_conf = np.load(conf_dsp_file)
        else:
            existing_conf = np.empty((0, 1))
        
        # Append new points.
        combined_xyz = np.vstack([existing_xyz, pts])
        combined_rgb = np.vstack([existing_rgb, col])
        combined_conf = existing_conf
        
        # Check if the combined point cloud exceeds max_pts_num.
        if combined_xyz.shape[0] > max_pts_num:
            print(f"Downsampling combined point cloud from {combined_xyz.shape[0]} to {max_pts_num}")
            # Normalize combined confidences.
            confs_min = np.min(combined_conf)
            confs_max = np.max(combined_conf)
            norm_conf = (combined_conf - confs_min) / (confs_max - confs_min) + 1
            weights = norm_conf.reshape(-1) / np.sum(norm_conf)
            indices = np.random.choice(combined_xyz.shape[0], max_pts_num, replace=False, p=weights)
            combined_xyz = combined_xyz[indices]
            combined_rgb = combined_rgb[indices]
            combined_conf = combined_conf[indices]
        # Re-write the PLY file and update the downsampled confidences.
        storePly(points3D_ply_file, combined_xyz, combined_rgb)
        np.save(conf_dsp_file, combined_conf)
    else:
        # If no PLY file exists yet, ensure the new batch is downsampled if needed.
        if pts.shape[0] > max_pts_num:
            print(f"Downsampling new points from {pts.shape[0]} to {max_pts_num}")
            confs_min = np.min(confs)
            confs_max = np.max(confs)
            norm_conf = (confs - confs_min) / (confs_max - confs_min) + 1
            weights = norm_conf.reshape(-1) / np.sum(norm_conf)
            indices = np.random.choice(pts.shape[0], max_pts_num, replace=False, p=weights)
            pts = pts[indices]
            col = col[indices]
    ...
    # Optionally save all original points and colors.
    if save_all_pts:
        pts3d_all_file = sparse_path / 'points3D_all.npy'
        pointsColor_all_file = sparse_path / 'pointsColor_all.npy'
        if pts3d_all_file.exists():
            existing_pts3d_all = np.load(pts3d_all_file)
            combined_pts3d_all = np.concatenate([existing_pts3d_all, pts3d], axis=0)
            np.save(pts3d_all_file, combined_pts3d_all)
        else:
            np.save(pts3d_all_file, pts3d)
        if pointsColor_all_file.exists():
            existing_color_all = np.load(pointsColor_all_file)
            combined_color_all = np.concatenate([existing_color_all, imgs], axis=0)
            np.save(pointsColor_all_file, combined_color_all)
        else:
            np.save(pointsColor_all_file, imgs)
            
        # Use the combined (appended) vanilla points as the "vanilla" count.
        vanilla_pts_all = np.load(pts3d_all_file)
        vanilla_pts_num = vanilla_pts_all.reshape(-1, 3).shape[0]
    else:
        # If not saving all points, use only the new batch.
        vanilla_pts_num = pts3d.reshape(-1, 3).shape[0]

    # Append statistics to pts_num.txt if a save path is provided.
    if save_txt_path is not None:
        if isinstance(save_txt_path, str):
            save_txt_path = Path(save_txt_path)
        pts_num_file = save_txt_path / 'pts_num.txt'
        with open(pts_num_file, 'a') as f:
            f.write(f"Depth threshold: {depth_threshold}\n")
